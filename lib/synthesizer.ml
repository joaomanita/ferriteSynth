...
type tm =
  ...
  | Func of ((id * (id * ty) list) * (ty * tm)) * string
  | App of tm * tm list
  | Fix of tm
  | Unfix of id * tm
  ...
  | SchemeFunc of (ty list * ((id * (id * ty) list) * (ty * tm))) * string
...
let rec subst e1 x e2 =
  ...
  | Func (((name, argList), (t, tm)), traits) ->
      ...
      else Func (((name, argList), (t, subst tm x e2)), traits)
...
let rec uses_typeR target ty =
  ...
  | TySendChannel (t1, t2) ->
      equal_type_with_existentials target t1 || uses_typeR target t2
  ...
  | TyAtomic _ | TyPrimitive _ -> equal_type_with_existentials target ty
...
let rec uses_typeL target ty =
  ...
  | TyReceiveChannel (t1, t2) ->
      equal_type_with_existentials target t1 || uses_typeL target t2
  | TySendValue (_, t1) -> uses_typeL target t1
  | TyReceiveValue (t1, t2) ->
      equal_type_with_existentials target t1 || uses_typeL target t2
  | TySharedToLinear (t1, _) | TyLinearToShared (t1, _) | TySession t1 -> (
      match target with
      | TySession t2 -> uses_typeL t2 t1
      | _ -> uses_typeL target t1)
  | TyRec t1 -> uses_typeL target t1
...
let rec print_inst_vars vars =
  match vars with
  | [] -> ""
  | (t1, t2) :: xs ->
      sprintf "(%s, %s); %s " (print_type t1) (print_type t2)
        (print_inst_vars xs)
...
let process_closed_function closed =
  match closed with
  | TyFunc (((name, argList), tRet), traits), body ->
      let t_resolved =
        resolve_type (TyFunc (((name, argList), tRet), traits))
      in
      fn_ctxt := (name, t_resolved) :: !fn_ctxt;
      Func (((name, argList), (tRet, RawText body)), traits)
  | TyScheme (scheme_list, TyFunc (((name, argList), tRet), traits)), body ->
      let t_resolved =
        resolve_type
          (TyScheme (scheme_list, TyFunc (((name, argList), tRet), traits)))
      in
      fn_ctxt := (name, t_resolved) :: !fn_ctxt;
...
let rec synthesize t argcountlist add_rec required_funcs usable_funcs =
  let name_func =
    match t with
    | TyFunc (((name, _), _), _)
    | TyScheme (_, TyFunc (((name, _), _), _))
    | TyUnitRetFunc (name, _) ->
        name
    | _ -> raise Fail
  in
  let delta_in =
  ...
  let gamma =
    List.map
      (fun (name, ty) -> ((name, ty), 1))
      (if add_rec then
         (name_func, resolve_type t)
         :: List.filter (fun (name, _) -> List.mem name usable_funcs) !fn_ctxt
       else List.filter (fun (name, _) -> List.mem name usable_funcs) !fn_ctxt)
  in
  argcounts := argcountlist;
  let t_resolved = resolve_type t in
  fn_ctxt := (name_func, t_resolved) :: !fn_ctxt;
  let programs =
    delay (fun () ->
...
and inversionR gamma delta_in omega t psi zeta theta focus_ctx ident =
  ...
  let tm =
    ...
    | TyFunc (((name, argList), tRet), traits) ->
        let argListCounted =
          List.map2 (fun (id, ty) count -> ((id, ty), count)) argList !argcounts
        in
        inversionR (argListCounted @ gamma) delta_in omega tRet psi zeta theta
          focus_ctx (ident + 1)
        >>= fun (ctxts_out, e) ->
        return
          ( ctxts_out,
            Func (((name, argList), (rev_resolve_type tRet, e)), traits) )
    | TyUnitRetFunc (name, argList) ->
        let argListCounted =
          List.map2 (fun (id, ty) count -> ((id, ty), count)) argList !argcounts
        in
        ...
        inversionR (argListCounted @ gamma) delta_in [] TyEnd psi zeta theta
          focus_ctx (ident + 1)
        >>= fun (ctxts_out, e) ->
        return (ctxts_out, UnitRetFunc ((name, argList), RunSession e))
    | TyScheme (tList, ty) -> (
        let subst =
          List.map
            (function
    ...
    | TyApp ret_ty -> (
        try
          let subst, ((id, func_ty), delta_in') =
            searchAndRemoveFuncType ret_ty delta_in []
          in
          let subst_func_ty = apply_all_subst func_ty subst in
          let name, tyArgList =
            match subst_func_ty with
            | TyFunc (((name, argList), _), _) -> (name, argList)
            | _ -> raise Fail
          in
          let focus_ctx' = searchAndRemoveFocusCtx focus_ctx id func_ty in
          let tyArgList = List.map (fun (_, t1) -> t1) tyArgList in
          let rec synth_args gamma delta theta focus_ctx = function
            | [] -> return ((gamma, ((delta, theta), focus_ctx)), [])
            | t :: rest ->
                let res =
                  try
                    let id, delta_out = searchAndRemove t delta in
                    let focus_ctx_out =
                      searchAndRemoveFocusCtx focus_ctx id t
                    in
                ...
                synth_args gamma' delta' theta' focus_ctx' rest
                >>=
                fun ((gamma'', ((delta'', theta''), focus_ctx'')), arg_tms) ->
                return
                  ( (gamma'', ((delta'', theta''), focus_ctx'')),
                    arg_tm :: arg_tms )
          in
    ...
    | TyReceiveChannel (tChan, tCont) ->
        ...
        inversionR gamma delta_in ((x, tChan) :: omega) tCont psi zeta theta
          focus_ctx (ident + 1)
        >>= fun (ctxts_out, e1) -> return (ctxts_out, ReceiveChannel (x, e1))
    | TyReceiveValue (tau, tCont) ->
        let x = fresh_val_id () in
        inversionR
          (((x, tau), 2) :: gamma)
          delta_in omega tCont psi zeta theta focus_ctx (ident + 1)
        >>= fun (ctxts_out, e1) -> return (ctxts_out, ReceiveValue (x, e1))
    | TyExternalChoice c -> (
        ...
        let out_ctxts =
          List.map (fun (_, (ctxts_out, _)) -> ctxts_out) branches
        in
        let out_deltas =
        ...
        if not (all_equal out_deltas) then (
          ...
          | ctxts_out :: _ ->
              ...
              in
              Choice.return (ctxts_out, OfferChoice choices))
  ...
  | (x, ty) :: xs ->
      ...
      let tm =
        ...
        | TySendChannel (tyChan, tyCont) ->
            ...
            >>= fun (ctxts_out, e1) ->
            return (ctxts_out, ReceiveChannelFrom (x, (binder, e1)))
        | TySendValue (tau, tyCont) ->
            ...
            >>= fun (ctxts_out, e1) ->
            return (ctxts_out, ReceiveValueFrom ((x, binder), e1))
        | TyInternalChoice c -> (
            let l = choice_to_list c in
            let branches =
              List.map
...
and decideFocus gamma delta_in t psi zeta theta focus_ctx ident =
  ...
  Choice.mplus (Choice.mplus l r) g
...
and can_add_focus_ctx focus_ctx t =
  ...
  | (_, ty) :: [] -> (
      match ty with
      | TyReceiveChannel (t1, _) -> equal_type_with_existentials t1 t
      | _ -> false)
  | _ -> false
...
and focusGamma gamma delta_in t psi zeta theta focus_ctx ident =
  let filtered_gamma =
    let rec aux = function
      | [] -> []
      | ((id, ty), timesUsed) :: xs ->
          if timesUsed <= 0 then aux xs else ((id, ty), timesUsed) :: aux xs
    in
    aux gamma
  in
...
and focusR gamma delta_in t psi zeta theta focus_ctx ident =
  ...
  let tm =
    ...
    | TyInternalChoice c ->
        ...
        >>= fun (ctxts_out, e) -> return (ctxts_out, Offer (label, e))
    | TySendChannel (t1, t2) -> (
        try
    ...
    | TyRec _ ->
        if List.mem t psi then
          ...
          inversionR gamma delta_in [] unfolded_t (t :: psi) zeta theta
            focus_ctx (ident + 1)
          >>= fun (ctxts_out, e) -> return (ctxts_out, Fix e)
    | TySharedToLinear (t1, counter) ->
        ...
        let result =
          if counter = 0 then
            ...
            inversionR gamma delta_in [] (TyApp t1) psi zeta theta focus_ctx
              (ident + 1)
        in
        result >>= fun (ctxts_out, e1) -> return (ctxts_out, Detach e1)
    | TyLinearToShared (t1, counter) ->
        let t2 = unfoldShared t in
        let result =
          if counter = 0 then
            ...
            inversionR gamma delta_in [] (TyApp t1) psi zeta theta focus_ctx
              (ident + 1)
        in
        result >>= fun (ctxts_out, e1) -> return (ctxts_out, Accept e1)
    | TyEnd ->
        log "%s success\n" (String.make ident ' ');
...
and searchAndRemoveFuncType t delta_in subst =
  match delta_in with
  | [] -> raise Fail
  | ((id, TyFunc (((_, _), retType), _)) as x) :: rest -> (
      try
        let subst = unifyT subst retType (TySession t) @ subst in
        (subst, ((id, snd x), rest))
      with CannotUnify ->
        let subst, (res, rest') = searchAndRemoveFuncType t rest subst in
        (subst, (res, x :: rest')))
  | x :: rest ->
      let subst, (res, rest') = searchAndRemoveFuncType t rest subst in
      (subst, (res, x :: rest'))
...
and focusL gamma delta_in t psi zeta theta focus_ctx ident =
  ...
  let r =
    delay (fun () ->
        ...
        focusL' gamma delta_in' id ty t psi zeta theta focus_ctx' (ident + 1))
  in
  if not (is_empty r) then (
...
and focusL' gamma delta_in id foc t psi zeta theta focus_ctx ident =
  ...
  let tm =
    ...
    | TyAtomic _ -> (
        ...
        | TyExistential _ -> (
            try
              let theta_out = unify (t, foc) theta in

              log "%s success existential unify\n" (String.make ident ' ');
    ...
    | TyRec _ ->
        ...
        let zeta =
          if searchZeta id foc zeta then zeta else ((id, foc), 0) :: zeta
        in

        log "%s\n" (string_of_int (searchTimesUsedZeta id foc zeta));
