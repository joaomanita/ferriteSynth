...
let rec resolve_type t =
  ...
  | TySchemeFunc (tList, ((name, argList), tRet)) ->
      TySchemeFunc
        ( tList,
          ( ( name,
              List.map
...
let rec contains_type target ty =
  ...
  | TyFunc ((_, _), tRet) -> contains_type target tRet
...
let rec uses_typeR target ty =
  ...
  | TyFunc ((_, _), tRet) -> uses_typeR target tRet
  | TyUnitRetFunc (_, argList) ->
...
let rec uses_typeL target ty =
  ...
  | TyFunc ((_, _), tRet) -> uses_typeL target tRet
  | TyUnitRetFunc (_, argList) ->
...
let process_closed_function closed =
  ...
  | TyFunc ((name, argList), tRet), body ->
      let t_resolved = resolve_type (TyFunc ((name, argList), tRet)) in
      fn_ctxt := (name, t_resolved) :: !fn_ctxt;
      Func ((name, argList), (tRet, RawText body))
  | TySchemeFunc (tList, ((name, argList), tRet)), body ->
      let t_resolved =
        resolve_type (TySchemeFunc (tList, ((name, argList), tRet)))
      in
      fn_ctxt := (name, t_resolved) :: !fn_ctxt;
      SchemeFunc (tList, ((name, argList), (tRet, RawText body)))
  | _ -> raise Fail

let rec synthesize t add_rec required_funcs usable_funcs =
  match t with
  | TyFunc ((name, _), _)
  | TyUnitRetFunc (name, _)
  | TySchemeFunc (_, ((name, _), _)) ->
      let delta_in =
        List.filter (fun (name, _) -> List.mem name required_funcs) !fn_ctxt
      in
      let gamma =
        List.map
          (fun (name, ty) -> ((name, ty), 0))
          (if add_rec then
             (name, resolve_type t)
             :: List.filter
                  (fun (name, _) -> List.mem name usable_funcs)
                  !fn_ctxt
           else
      ...
      let programs =
        delay (fun () ->
            ...
            >>= fun (((delta_out, theta_out), focus_ctx'), e) ->
            if delta_out <> [] then (
              print_fail "synthesize" 0;
              Choice.fail)
            else return (((delta_out, theta_out), focus_ctx'), e))
      in
      run_n 3 programs
  | _ -> raise Fail
  ...
  let tm =
    ...
    | TyFunc ((name, argList), tRet) ->
        ...
        inversionR (argListCounted @ gamma) delta_in omega tRet psi zeta theta
          focus_ctx (ident + 1)
        >>= fun (ctxts_out, e) ->
        return (ctxts_out, Func ((name, argList), (rev_resolve_type tRet, e)))
    | TySchemeFunc (tList, ((name, argList), tRet)) -> (
        let _tList =
          List.map
            (function
        ...
        inversionR gamma delta_in omega
          (TyFunc ((name, _argList), _tRet))
          psi zeta theta focus_ctx ident
        >>= fun (ctxts_out, e) ->
        match e with
        | Func ((_, _), (_, e1)) ->
            return (ctxts_out, SchemeFunc (tList, ((name, argList), (tRet, e1))))
        | _ -> raise Fail)
    | TyUnitRetFunc (name, argList) ->
        let argListCounted = List.map (fun (id, ty) -> ((id, ty), 0)) argList in
    ...
    | TyApp ty ->
        let name, tyArgList =
          match ty with
          | TyFunc ((name, argList), _) | TySchemeFunc (_, ((name, argList), _))
            ->
              (name, argList)
          | _ -> raise Fail
        in
        let tyArgList = List.map (fun (_, t1) -> t1) tyArgList in
        let rec synth_args delta theta focus_ctx = function
          | [] -> return (((delta, theta), focus_ctx), [])
          | t :: rest ->
              let res =
                try
                  let id, delta_out = searchAndRemove t delta in
                  let focus_ctx_out = searchAndRemoveFocusCtx focus_ctx id t in
                  return (((delta_out, theta), focus_ctx_out), Var id)
                with Fail ->
                  inversionR gamma delta omega t psi zeta theta focus_ctx
                    (ident + 1)
              in
              res >>= fun (((delta', theta'), focus_ctx'), arg_tm) ->
              synth_args delta' theta' focus_ctx' rest
              >>= fun (((delta'', theta''), focus_ctx''), arg_tms) ->
              return (((delta'', theta''), focus_ctx''), arg_tm :: arg_tms)
        in
        synth_args delta_in theta focus_ctx tyArgList
    ...
    let rec aux seen = function
      ...
      | ((id, ty), timesUsed) :: xs ->
          if
            timesUsed >= 2
            || List.exists
                 (fun (seenId, seenT) -> seenId = id || equal_type seenT ty)
                 seen
          then aux seen xs
          else ((id, ty), timesUsed) :: aux ((id, ty) :: seen) xs
    in
    aux [] gamma
  in
...
and focusR gamma delta_in t psi zeta theta focus_ctx ident =
  ...
  let tm =
    ...
    | TySendValue (tau, t2) ->
        ...
        >>= fun (((delta_out1, theta_out1), focus_ctx'), e1) ->
        inversionR gamma delta_out1 [] t2 psi zeta theta_out1 focus_ctx'
          (ident + 1)
        >>= fun (((delta_out2, theta_out2), focus_ctx'), e2) ->
        return (((delta_out2, theta_out2), focus_ctx'), SendValue (e1, e2))
    | TyRec _ ->
    ...
    | TyEnd ->
        if delta_in <> [] then (
          ...
          log "%s success\n" (String.make ident ' ');
          return (((delta_in, theta), focus_ctx), Terminate))
      ...
      try
        ...
        let argList =
          List.map
            ...
            argList
        in
        let func_ty = TyFunc ((name, argList), apply_all_subst retType subst) in
        ((id, func_ty), rest)
      with CannotUnify ->
        let res, rest' = searchAndRemoveFuncType t rest in
        (res, x :: rest'))
  ...
  let tm =
    ...
    | TyAtomic _ ->
        if equal_type foc t then (
          log "%s success\n" (String.make ident ' ');
          return (((delta_in, theta), focus_ctx), Forward id))
        else (
          print_fail "focusL" ident;
          Choice.fail)
    | TyExistential _ -> (
    ...
    | TyPrimitive _ ->
        if equal_type foc t then (
          log "%s success\n" (String.make ident ' ');
          return (((delta_in, theta), focus_ctx), Var id))
        else (
          print_fail "focusL" ident;
          Choice.fail)
    | TySession t1 ->
    ...
    | TyFunc ((_, _), TySession tRet) ->
        if equal_type tRet t then
          ...
          inversionR gamma delta_in [] (TyApp foc) psi zeta theta focus_ctx
            (ident + 1)
          >>= fun (((delta', theta'), focus_ctx'), cutL) ->
          let cut_dirs =
            List.map
              (fun (id, ty) ->
                if
    ...
    | TyReceiveValue (t1, t2) ->
        ...
        >>= fun (((delta_out2, theta_out2), focus_ctx''), e2) ->
        return
          (((delta_out2, theta_out2), focus_ctx''), SendValueTo ((id, e1), e2))
